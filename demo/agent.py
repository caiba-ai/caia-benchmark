...
from demo.utils import INSTRUCTIONS_PROMPT, merge_statistics
...
class Agent(ABC):
        ...
        if tool_calls:
            for tool_call in tool_calls:
                ...
                if tool_name not in self.tools:
                    ...
                    continue
                tool_call_metadata['tool_description'] = self.tools[tool_name].description
                ...
                if tool_result["success"]:
                    ...
                    tool_call_metadata['tool_output'] = tool_result["result"]
                else:
    ...
    async def run(self, question: str, session_id: str = None) -> tuple[str, dict]:
        ...
        messages = [
            {
                "role": "user",
                "content": self.instructions_prompt.format(date = datetime.now().strftime('%B %d, %Y'),question=question),
            }
        ]
