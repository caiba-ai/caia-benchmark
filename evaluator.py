...
2. If the tool_output is not related to the question, just summarize the tool_output to "No relevant information Found"
        ...
        async def process_tool_use(tool_use: ToolUse) -> ToolUse:
            if tool_use.tool_output and count_tokens(tool_use.tool_output, self.parse_model) > self.tool_output_max_tokens:
                user_prompt = f"""
                Question: {question}
                Tool use:
                {tool_use.to_prompt()}
...
"""
        ...
        if 'deepseek' in self.model_name:
            if count_tokens(prompt) >= DEEPSEEK_MAK_CONTEXT_TOKEN:
                summarized_tool_use_list = await self.summarize_tool_use_output(question=benchmark_item.question, tool_use_list=output_answer.function_calls)
                prompt = f"""
Task ID: {benchmark_item.task_id}
Question: {benchmark_item.question}
To be evaluated tool use:
...
"""
    ...
    async def analyze_evaulate_result(self, 
        ...
        benchmark_answer_items = [item for item in to_evaluate_item.evaluate.items if item.target == EvaluateTarget.ANSWER]
        benchmark_answer_item = benchmark_answer_items[0] if benchmark_answer_items else None
        ...
        # Answer part
        answer_score = answer_evaulate_result.score if answer_evaulate_result and hasattr(answer_evaulate_result, "score") else 0
        answer_reason = answer_evaulate_result.reason if answer_evaulate_result and hasattr(answer_evaulate_result, "reason")  else ""
        answer_total_score = benchmark_answer_item.points if benchmark_answer_item else 0
        detail += f"Answer score: {answer_score} / {answer_total_score}\n"
        detail += f"Reason: {answer_reason}\n"

        # Reasoning part
        reasoning_items = reasoning_evaulate_result.items if reasoning_evaulate_result and hasattr(reasoning_evaulate_result, "items") and reasoning_evaulate_result.items else []
        reasoning_score = sum([item.score for item in reasoning_items]) if reasoning_items else 0
        reasoning_total_score = sum([item.points for item in benchmark_reasoning_items]) if benchmark_reasoning_items else 0
        ...
        for idx, item in enumerate(reasoning_items):
            ...
            points = benchmark_reasoning_items[step-1].points if step-1 < len(benchmark_reasoning_items) else 0
            detail += f"Reasoning step {step}: {reason} score: {score} / {points}\n"

        # Tool use part
        tool_use_items = tool_use_evaulate_result.items if tool_use_evaulate_result and hasattr(tool_use_evaulate_result, "items") and tool_use_evaulate_result.items else []
        tool_use_score = sum([item.score for item in tool_use_items]) if tool_use_items else 0
        tool_use_total_score = sum([item.points for item in benchmark_tool_use_items]) if benchmark_tool_use_items else 0
